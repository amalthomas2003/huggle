...
const FindGroomer = () => {
  ...
  const getGroomerCategory = (name = "") => {
  const lower = name.toLowerCase();

  if (lower.includes("groom")) {
    return { label: "Pet Grooming", color: "bg-pink-100 text-black-800" };
  }
  if (lower.includes("training") || lower.includes("trainer")) {
    return { label: "Dog Training", color: "bg-yellow-100 text-black-800" };
  }
  if (lower.includes("aquarium") || lower.includes("fish")) {
    return { label: "Aquarium", color: "bg-purple-100 text-black-800" };
  }
  return { label: "Pet Store", color: "bg-cyan-100 text-black-800" };
};
...
// Otherwise, make a fresh API call after cooldown
      ...
      try {
        ...
        setError("Failed to fetch vet services.");
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="min-h-screen bg-orange-50 py-6 px-4 md:px-10">
      ...
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold text-gray-800">Find Groomers/Services Nearby</h1>
        <button
    ...
    >
      ...
      {loading && <p>Loading pet services near you...</p>}
  ...
  className="flex flex-col justify-between h-full relative bg-white p-4 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300"
            >
              ...
              />
              {(() => {
const { label, color } = getGroomerCategory(vet.displayName?.text);
  return (
    <p className={`inline-block text-xs px-2 py-1 rounded-full font-medium ${color} mb-2`}>
      {label}
              ...
              <h2 className="text-lg font-bold text-gray-800 mb-1">
                {vet.displayName?.text || "Unnamed Service"}
              </h2>
